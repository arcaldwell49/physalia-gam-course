...
library(ggdist)
parse_dist(pr) %>% janitor::clean_names() %>% ggplot(aes(y = prior, xdist = dist, args = args)) +
  stat_halfeye()


priors =  c(prior(normal(0,1), class = "b" ,coef = "Year"),
  prior(lognormal(0,1), class = "sigma"))

priors %>%
  parse_dist(prior) %>%
  janitor::clean_names() %>%
  ggplot(aes(y = prior, xdist = dist, args = args)) +
  stat_halfeye() +
  labs(
    title = "stat_halfeye()",
...
pr_draws1 <- data.frame(t(p_year1)) %>%  # convert to a df transposed(!)
  ...
  tibble::add_column(Year = gtemp[["Year"]],     # add a column for the data: Year
             .before = 1L) %>%
...
prior_pred <- predict(m_prior2) %>%
  as_tibble() %>%
  tibble::add_column(Temperature = gtemp$Temperature, .before = 1L)
...
pr2 <-  c(prior(student_t(5, 0, 2.5), class = "b"),
          prior(student_t(5, 0, 2), class = "b", coef = "sYear_1"),
          prior(student_t(10, 0, 2), class = "sds"))

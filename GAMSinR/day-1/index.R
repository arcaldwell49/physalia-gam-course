...
wasp <- read.csv(here("data", "darlingtonia.csv"), comment.char = "#") %>%
  as_tibble()
...
p_gaus <- pdat %>%
  bind_cols(data.frame(p_gaus)) %>%
  mutate(fitted = ilink(fit),
         upper = ilink(fit + (qnorm(.975) * se.fit)),
         lower = ilink(fit - (qnorm(.975) * se.fit)))
# plot
p2 <- ggplot(maddy, aes(x = midDepth, y = calMid)) +
    geom_ribbon(aes(ymin = lower, ymax = upper,
                    x = midDepth), data = p_gaus,
...
pred <- bind_cols(newd, pred) %>%
    mutate(upr = fit + qnorm(.975) * se.fit, lwr = fit - 2*se.fit)
